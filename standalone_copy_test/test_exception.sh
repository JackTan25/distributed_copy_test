...
for ((i=1; i<=1000; i++)); do
    ...
    log_command bendsql --dsn "$DSN"  -q \"copy into products_ from @s1_ force = true purge = true\"
    ...
    if [[ $last_string1 != $expected ]]; then
     ...
     # 测试 from query
    log_command bendsql --dsn "$DSN"  -q \"copy into products_ from \(select \\\$1,\\\$2,\\\$4 from @s1_ as t2\) force = true purge=true\"
