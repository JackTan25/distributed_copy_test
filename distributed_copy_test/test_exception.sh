...
for ((i=1; i<=1000; i++)); do
    ...
    log_command bendsql --dsn "$DSN"  -q \"copy into products from @s1 force = true\"
    ...
    if [[ $last_string1 != $expected ]]; then
     ...
     # 测试 from query
    log_command bendsql --dsn "$DSN"  -q \"copy into products from \(select \\\$1,\\\$2,\\\$4 from @s1 as t2\) force = true\"
